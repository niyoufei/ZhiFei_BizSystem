...
前端“Ollama 增强预览”区域支持在生成预览后进行人工复制和导出：

- “复制预览结果”按钮复制当前页面展示的预览文本，包含 `enhanced_by`、`fallback`、`error_summary`、更新时间、高分逻辑预览和编制指导预览。
- “导出 JSON”按钮导出本次预览 API 返回的 JSON。
- 两个按钮只使用浏览器内存中的最近一次预览结果，不重新触发评分。
- 两个按钮不写入 `evolution_reports`。
- 两个按钮不修改评分分数。
- 两个按钮不调用 `scorer` / `v2_scorer`。
- 两个按钮不写数据库。
